...
SCRIPT=$(readlink_own -m $SCRIPT_FOLDER/identifyStrain.sh)
...
# real calls
#testExitCode "/bin/bash $SCRIPT -i $TEST_DATA_FOLDER/test.file" "$EXIT_OK" "Simple compression test" "$TMP_OUT/identifyStrain1.test"
# TODO: add real calls

# delete all the temporary file
# TODO: needed ?
#rm -f $TMP_OUT/identifyStrain*.test 2>&1 > /dev/null

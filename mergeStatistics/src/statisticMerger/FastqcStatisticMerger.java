...
public class FastqcStatisticMerger extends AbstractStatisticMerger {
	...
	private static final String MODULE_TSQ = "Per tile sequence quality";
	...
	private static final String MODULE_AC = "Adapter Content";
	private static final String MODULE_KC = "Kmer Content";
    ...
    static {
        ...
        MODULE_NAMES.add(MODULE_AC);
        MODULE_NAMES.add(MODULE_KC);
	...
	protected boolean extractTable(String moduleName, String file, BufferedWriter outfile, boolean writeHeader) {
		boolean isStatusMode = false; // not all all modules should be extracted, it's done one by one.
				...
				// is in correct module --> write lines
					...
					if(!isStatusMode && line.equals(MODULE_PREFIX + MODULE_END)) 
						...
						if(line.startsWith(HEADER_PREFIX)) {
							if(MODULE_SDL.equals(moduleName) && skippedHeader == false) {
								skippedHeader = true;
								continue;
							}
							if(writeHeader) {

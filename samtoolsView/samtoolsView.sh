...
DEFINE_boolean 'cramoutput' '1' "output CRAM (requires -T)" 'C'
DEFINE_boolean 'fastCompression' '1' "use fast BAM compression (implies -b)" '1'
DEFINE_boolean 'uncompressedBam' '1' "uncompressed BAM output (implies -b)" 'u'
...
DEFINE_integer 'mappingquality' '0' "only include reads with mapping quality >= INT" 'q'
DEFINE_string 'library' '' "only include reads in library STR" 'l'
DEFINE_integer 'minquerylength' '' "only include reads with number of CIGAR operations consuming  query sequence >= INT" 'm'
...
DEFINE_float 'seed' '0' "-s FLOAT integer part sets seed of random number generator   rest sets fraction of templates to subsample" 's'
DEFINE_string 'threads' '' "-@, --threads INT  number of BAM/CRAM compression threads" '@'
DEFINE_string 'printLongHelp' '' "print long help, including note about region specification" 'h2'
DEFINE_string 'inputfmtoption' '' "--input-fmt-option OPT  Specify a single input file format option in the form  of OPTION or OPTION=VALUE" ''
DEFINE_string 'outputfmt' '' "-O, --output-fmt FORMAT  Specify output format (SAM, BAM, CRAM)" 'O'
DEFINE_string 'outputfmtoption' '' "--output-fmt-option OPT  Specify a single output file format option in the form  of OPTION or OPTION=VALUE" ''
DEFINE_string 'reference' '' "-T, --reference FILE  Reference sequence FASTA FILE" 'T'

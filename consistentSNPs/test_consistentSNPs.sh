...
SCRIPT=$(readlink_own -m $SCRIPT_FOLDER/consistentSNPs.sh)
...
# real calls
#testExitCode "/bin/bash $SCRIPT -i $TEST_DATA_FOLDER/test.file" "$EXIT_OK" "Simple compression test" "$TMP_OUT/consistentSNPs1.test"
# TODO: add real calls

# delete all the temporary file
# TODO: needed ?
#rm -f $TMP_OUT/consistentSNPs*.test 2>&1 > /dev/null
